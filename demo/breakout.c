...
rgb_color_t ball_color = (rgb_color_t){1, 0, 0};
...
void on_key(char key, key_event_type_t type, double held_time, state_t *state) {
  if (type == KEY_PRESSED) {
    switch (key) {
    case LEFT_ARROW: {
      if (body_get_velocity(state->user).x == VEC_ZERO.x) {
        body_set_velocity(state->user,
                          vec_negate((vector_t){resting_speed, 0}));
      } else {
        body_set_velocity(
            state->user,
            (vector_t){.x = body_get_velocity(state->user).x - ACCEL,
                       .y = VEC_ZERO.y});
      }
      break;
    }
    case RIGHT_ARROW: {
      if (body_get_velocity(state->user).x == VEC_ZERO.x) {
        body_set_velocity(state->user, (vector_t){resting_speed, 0});
      } else {
        body_set_velocity(
            state->user,
            (vector_t){.x = body_get_velocity(state->user).x + ACCEL,
...
void add_user(state_t *state) {
  ...
  body_t *user_body = body_init_with_info(user_shape, INFINITY, user_color,
                                          make_type_info(WALL), free);
...
state_t *emscripten_init() {
  ...
  add_ball(state);
  add_user(state);
  state->ball = scene_get_body(state->scene, 0);
  state->user = scene_get_body(state->scene, 1);
