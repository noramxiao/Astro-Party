...
static vector_t get_max_min_projections(list_t *shape, vector_t unit_axis) {
  ...
  for (size_t i = 0; i < list_size(shape); i++) {
    ...
    // component of vector along a unit vector is just dot product
    double component = vec_dot(*point, unit_axis);
    if (component > max) {
      max = component;
    }
    if (component < min) {
...
static collision_info_t compare_collision(list_t *shape1, list_t *shape2,
  ...
  list_t *edges = get_edges(shape1);
  vector_t collision_axis = VEC_ZERO;

  for (size_t i = 0; i < list_size(edges); i++) {
    vector_t *edge = list_get(edges, i);
    vector_t axis = vec_unit((vector_t){-edge->y, edge->x});
    vector_t projections1 = get_max_min_projections(shape1, axis);
    vector_t projections2 = get_max_min_projections(shape2, axis);
